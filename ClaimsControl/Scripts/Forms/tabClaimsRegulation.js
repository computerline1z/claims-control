...
(function() {
  ...
  App.tabClaimsRegulationController = Em.ArrayController.create({
    ...
    setActivitiesTable: function() {
      var activities, activitiesTbl, claimID;

      claimID = this.claim.iD;
      console.log("Showing all activities Claim iD: " + claimID);
      activities = oDATA.GET("proc_Activities").emData.filter(function(item) {
    ...
    setFinancesTables: function() {
      var claimID, finTypes, finances;

      claimID = this.claim.iD;
      console.log("Setting finances table");
      finances = oDATA.GET("proc_Finances").emData.filter(function(item) {
        return item.claimID === claimID;
      });
    ...
    saveForm: function(e) {
      var Action, ClaimID, DataToSave, addData, context, form, formOpts, me;
      ...
      if (DataToSave) {
        return SERVER.update2({
          ...
          CallBackAfter: function(Row) {
            me.cancelForm();
            if (context.activityTypeID) {
              return me.setActivitiesTable();
            } else {
              return me.setFinancesTables();
            }
          }
        });
      }
      ...
      didInsertElement: function() {
        ...
        if (this.isNew) {
          switch (this.activityTypeID) {
            ...
            case 4:
            case 5:
              divExt = $(this.frm).find("div.row").find("div.ExtendIt:first");
          ...
          (function(iD) {
            this.NewRec = 0;
            return this.id = iD;
          }).call($(this.frm).data("ctrl"), this.iD);
          this.set("deleteButton", true);
        }
        return oCONTROLS.UpdatableForm({
          frm: this.frm,
          btnSaveToDisable: $(this.frm).find("button.btnSave")
        });
      },
  ...
  App.TabClaimsRegulationView = Ember.View.extend(App.HidePreviousWindow, {
    ...
    didInsertElement: function() {
      ...
      $("#claimRegulationTab").tabs().on("tabsactivate", function(event, ui) {
