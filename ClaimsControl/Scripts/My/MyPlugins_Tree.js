...
(function() {
  var w=window,$ = w.jQuery, App=w.App, Em=w.Em, initTreePadding = false;  App.create_docsTypesController = function(categoryOpts) {
    return App.docsTypesController = Em.ResourceController.create({
      ...
      refreshTree: function(categoryOpts) {
        var c, catOpts, cats, docs, me, newItem_Nepriskirti;

        oGLOBAL.logFromStart("Started refreshTree");
        if (categoryOpts) {
          this.set("categoryOpts", categoryOpts);
        } else {
          categoryOpts = this.categoryOpts;
        }
        ...
        cats.forEach(function(catItem, i) {
          var No, catDocs, fnGetCatDocs, newItem, ref, refID;
          ...
          fnGetCatDocs = function(refID, item) {
            var catDocs;

            catDocs = docs.filter(function(doc) {
              return doc.refID === refID && doc.groupID === catItem.iD;
            });
            item.title += catDocs.length ? " (" + catDocs.length + ")" : "";
            return catDocs;
          };
          if (ref === 4) {
            No = 0;
            ...
            newItem.items.forEach(function(vehicle) {
              ...
              catDocs = fnGetCatDocs(vehicle.refID, vehicle);
              No += catDocs.length;
              return vehicle.items = me.setDocTypes(me.docTypes, catItem.iD, vehicle.refID, catDocs);
            });
            newItem.title += No ? " (" + No + ")" : "";
          } else {
            refID = ref === 3 ? catOpts.driver.iD : catOpts.accident.iD;
      ...
      setDocTypes: function(docTypes, groupID, refID, catDocs) {
        var No;
        ...
        return docTypes.filter(function(type) {
          ...
          No = catDocs.filter(function(doc) {
            return doc.docTypeID === docType.iD;
          }).length;
          No = No ? " (" + No + ")" : "";
          return {
            categoryId: docType.iD,
            title: docType.name + No,
            refID: refID
          };
        });
      },
  ...
  $.widget("ui.Tree", {
    ...
    TreeDocControllerOpts: {
      ...
      refreshDocs: function() {
        ...
        return oGLOBAL.logFromStart("Finished refreshDocs");
      },
      docs: [],
      opts: null,
      AllDocs: [],
...
/*
//@ sourceMappingURL=MyPlugins_Tree.map
*/
