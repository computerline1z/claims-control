...
_create: ->	
	...
	if opt.Editable.Add
		...
		opt.appendToList="<span style='margin:-22px 2px auto auto;' title='redaguoti..' class='ui-icon ui-icon-pencil ui-menu-icon'>&nbsp;</span>"
...
#=======================================================================================
$.widget "ui.ComboBoxWithCategory", $.ui.ComboBox, 
	_create:() -> 
		@_super();		
		@element.data("autocomplete")._renderMenu = (ul, items) ->
			that = this; currentCategory = "";
			$.each( items, ( index, item ) ->
				# if ( item.category != currentCategory )
					# ul.append( "<li class='ui-autocomplete-category'>" + item.category + "</li>" );
...
###		
	_renderMenu: (ul, items) ->
		...
		# unless item.category is currentCategory
			# ul.append "<li class='ui-autocomplete-category'>" + item.category + "</li>"
			# currentCategory = item.category
			ul.append "<li class='ui-autocomplete-category'>opa OPa</li>"
			that._renderItemData ul, item
###					
###			
 $(input).data("autocomplete")._renderMenu = (ul, items) ->
