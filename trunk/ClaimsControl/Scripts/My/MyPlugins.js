...
_init : function() {//type:Date,Integer,Decimal
	...
	var fnMain=function(e){
		var blur=((e.type==='blur')?'Blur':''), input=$(e.target),text=input.val(),len=text.length,key=e.charCode,str=String.fromCharCode(e.keyCode),pos=input.caret(),opts=this.opts,e=e;
		if (!blur){//if keypress
			var sel=this.fnHandlers.getSelection(input[0]);//console.log("sel.start:"+sel.start+", sel.end:"+sel.end);
