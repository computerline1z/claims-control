...
(function() {
  ...
  App.SelectedAccidentView = Em.View.extend({
    ...
    init: function() {
      this._super();
      return App.thisAccidentController.setContent(this);
    },
    tbodyClick: function(e) {
      ...
      MY.tabAccidents.SelectedClaimView = App.SelectedClaimView.create({
        ...
        contentBinding: 'App.claimEditController.oldClaim'
      });
      ...
      d = {
        ...
        fnAfterOptClick: function(T) {
          ...
          MY.tabAccidents.NewClaimView = App.SelectedClaimView.create({
            ...
            contentBinding: 'App.claimEditController.newClaim'
          });
  ...
  App.SelectedClaimView = Em.View.extend({
    didInsertElement: function() {
      var IClaim, btnSaveToDisable, c, cnt, fnCheckIsInjured, fnToggle_noInsurance, frm, inputs, isInjured, thisCtrl, typeID;
      c = this.rowContext;
      ...
      if (typeID === 2) {
        IClaim = thisCtrl.find(".js-InsuranceClaimAmount").parent().parent();
        ...
        isInjured = thisCtrl.find('.js-IsInjuredPersons');
        fnCheckIsInjured.call(isInjured);
        isInjured.on("click", function() {
          return fnCheckIsInjured.call(isInjured);
        });
      }
      if (typeID === 6) {
        thisCtrl.find(".js-claimEditDays, .js-claimEditPerDay").on("keyup", $.proxy(function() {
          return this.find('.js-lossAmount').setVal(this.find(".js-claimEditDays").getVal() * this.find(".js-claimEditPerDay").getVal());
        }, thisCtrl));
      }
      if (c.claimStatus > 2) {
        ...
        $(frm).find("input").filter(".js-lossAmount, .js-claimEditDays");
        if (c.claimStatus === "3") {
          inputs = inputs.filter(".js-lossAmount").prop('title', 'Suma jau patvirtina');
        } else if (c.claimStatus === "4") {
          inputs = inputs.filter(".js-lossAmount, .insuranceClaimAmount").prop('title', 'Suma jau patvirtina');
        } else if (c.claimStatus === "5") {
          $(frm).parent().find("button").prop("disabled", true);
          inputs.prop('title', 'Žala uždaryta');
        }
        inputs.prop("disabled", true);
      }
      fnToggle_noInsurance = function(content, isClicked) {
        var chk, claimAmount, eToggle, noInsurance;
        ...
        if (isClicked) {
          this.find("button.btnSave").attr("disabled", false);
        }
        eToggle = this.find("div.js-toggle");
        if (noInsurance) {
          ...
          eToggle.show();
          this.find("js-InsuredClaimList").data("newval", "");
        }
        isInjured = this.find('.js-IsInjuredPersons');
        if (isInjured.length) {
          claimAmount = this.find('.js-InsuranceClaimAmount').closest('.ExtendIt');
          if (isInjured.prop("checked")) {
            claimAmount.show();
          } else {
            claimAmount.hide();
          }
      ...
      cnt = this.content;
      frm = $("#" + this.elementId);
      cnt.addObserver('noInsurance', function() {
        return fnToggle_noInsurance.call(frm, cnt, true);
      });
      return fnToggle_noInsurance.call(frm, cnt, false);
    },
    init: function() {
  ...
  App.claimEditController = Em.Controller.create({
    ...
    saveForm: function(e) {
      var Action, DataToSave, Msg, accidentID, clickedCtrl, fnAfterUpdate, frm, newClaim, opt, tr;
      this.goToLastClick();
      ...
      if (DataToSave) {
        ...
        fnAfterUpdate = function(resp, updData) {
          ...
          App.thisAccidentController.setContent_afterEdit({
            accidentID: rowArr[0],
            claimID: id,
            Action: updData.Action,
            cancelCtrl: clickedCtrl.next()
          });
          return false;
        };
        opt = {
    ...
    goToLastClick: function() {
      if (this.scroolBack) {
        return Em.run.next(this.scroolBack, function() {
          this.tr[0].scrollIntoView();
          this.tr.trigger("click");
          if (this.div) {
            this.div[0].remove();
          }
          App.claimEditController.scroolBack = null;
          return false;
        });
      }
    }
  ...
  App.accidentsController = Em.ResourceController.create({
    ...
    tbodyClick: function(e) {
      var AddWr, button, div, offset, parent, tr;
      tr = $(e.target).closest("div.tr");
      button = $("#accidentsTable").find("button.btnSave");
      if (button.length) {
        if (!button.prop("disabled")) {
          offset = button.parent().offset();
          button.parent()[0].scrollIntoView();
          ...
          div.fadeIn("slow").click(function() {
            return this.remove();
          });
          App.claimEditController.scroolBack = {
            tr: tr,
            div: div
          };
          return false;
  ...
  App.thisAccidentController = Em.ResourceController.create({
    ...
    getContent: function(context) {
      var ArrClaims, ArrClaims2, clTypeID, i, iterator;
      console.log("getContent CONTEXT:");
      console.log(context);
      var ArrView = [],objView=[],claimsTypeID=context.claims_TypeID.split('#');
      ArrClaims = context.claims_C.replace(new RegExp('{{(.*?)}}', 'gm'), '').split('#||');
      ...
      if (ArrClaims[0] !== "") {
        ...
        while ((i++) < iterator) {
          ...
          objView[i] = Em.Object.create({
            ...
            claimID: parseInt(ArrView[i].Claims2[0], 10),
            accidentID: context.iD,
            accidentDate: context.date
          });
        }
      }
      return {
    ...
    setContent: function(context) {
      var obj;
      obj = this.getContent(context);
      return this.set("content", obj.objView).set("accidentID", obj.accidentID);
    },
    setContent_afterEdit: function(p) {
      var accident, newClaim, objView;
      accident = App.accidentsController.content.findProperty('iD', p.accidentID);
      objView = this.getContent(accident).objView;
      if (p.Action === "Add") {
        this.content.pushObject(objView[objView.length - 1]);
      } else {
        newClaim = objView.findProperty("claimID", p.claimID);
        this.content.findProperty("claimID", p.claimID).updateTo(newClaim);
      }
      p.cancelCtrl.trigger('click');
      return false;
    }
  });
